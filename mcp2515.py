...
class CAN (object):
    def __init__(self, spi, cs):
        ...
        # Software reset
        self._spi_Reset()
        # If you can read the data, it is considered that there is no problem
        # with initialization. At least the chip was soldered.
        time.sleep(0.2)
    ...
    def Start(self, Frequency, SpeedCfg, Filter=None, ListenOnly=False):
        ...
        #     5K, 10K, 20K, 33K, 40K, 50K, 80K, 95K,
        #     100K, 125K, 200K,  250K, 500K, 1000K
    ...
    def Send_msg(self, msg, sendchangel=None):
        ...
        # 1: channel 1
        # 2: channel 2
        # MCP2515 provides three sending channels, and channel 0 is used
        # by default.
        # Follow up to automatically find free channels, so stay tuned.
        # Note: If there is a message to be sent in the channel, the previous
    ...
    def Recv_msg(self):
        # Function: Inquire whether the MCP2515 has received a message. If
        # there is, it is stored in Buf. CheckRx is called.
        # Query whether Buf has a message. If yes, return the earliest
        # received frame, otherwise return None.
        # Return to Msg description:
        # msg['tm']: Time to receive the message. Timer since power on,in ms.
    ...
    def CheckRx(self):
        # Function: Inquire whether the MCP2515 has received a message.
        # If there is, save Buf and return TRUE, otherwise return False.
        # Note: Failure to store the messages in the MCP in Buf in time
        # may cause the MCP to fail to receive new messages.
    ...
    def Pin_RXxBF_as_Output(self, pin, value=None):
        ...
        # Other: invalid
        # Note: After calling this method, even if the port was originally
        # in the receive message interrupt mode, the port will be set to output.
    ...
    def Pin_TXxRTS_as_Input(self, pin):
        ...
        # Empty: Returns the status of the port. 1--High level 0--Low level
        # Other: invalid
        # Note: After calling this method, the port will be set to input even
        # if the port was originally in the interrupt mode for sending packets.
        p = {2: 0x20, 1: 0x10, 0: 0x08}.get(
            pin)  # Port is invalid? Don't care!
        rst = self._spi_ReadReg(b'\x0d')
        return 1 if ((rst[0] & p) != 0) else 0

    def _MsgVerificationCheck(self, msg):
        # Function: Check whether the content of msg is a CAN data format
        # that meets the sending requirements.
        ...
        if ((msg.get('ext') == False) and (msg.get('id') > 0x7ff)):
            raise Exception(
                'Msg format error: id exceeds the maximum value of the standard frame.')
        if ((msg.get('ext') == True) and (msg.get('id') > 0x1fffffff)):
            raise Exception(
                'Msg format error: id exceeds the maximum value of the extended frame.')
        if (len(msg.get('data', b'')) < msg.get('dlc')):
            raise Exception(
                'Msg format error: data is shorter than the length defined by dlc.')
        if (msg.get('dlc') > 8):
            raise Exception(
                'Msg format error: dlc length cannot be greater than 8.')
